...
def test(image,
    ...
    image_resize = cv2.resize(image, target_shape) / 255.0
    image_exp = np.expand_dims(image_resize, axis=0).astype("float32") #batch_size = 1
...
class Detector():
    ...
    def checkMask(self, raw):
        tic = timer()
        bytes_stream = BytesIO(raw)
        roiimg = Image.open(bytes_stream)
        img = cv2.cvtColor(np.asarray(roiimg), cv2.COLOR_RGB2BGR)
        ans, picture = test(img, target_shape=(260, 260), anchors=self.anchors, interpreter=self.interpreter)
        toc = timer()
        print(toc - tic)
        print(ans)
        return ans, picture
